...
def normalize_phone_number(phone_number_str):
    try:
        ...
        phone_number_str = ''.join(char for char in phone_number_str if char.isdigit() or char in '+')
...
def validate_row_numbers(row):
    if None in (normalize_phone_number(row['primary_number']), normalize_phone_number(row['secondary_number'])):
        return False
    return True
